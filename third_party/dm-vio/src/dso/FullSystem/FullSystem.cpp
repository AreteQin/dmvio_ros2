...
namespace dso
{
    ...
    Sophus::SE3d FullSystem::PublishPose(bool onlyLogKFPoses, bool saveMetricPoses)
    {
        ...
        if (saveMetricPoses)
        {
            ...
            camToFirst = Sophus::SE3d(
                imuIntegration.getTransformDSOToIMU().transformPose(
                    dmvio::PoseTransformation::PoseType(camToWorld.inverse().matrix())));
        }
    ...
    void
    FullSystem::printResult(std::string file, bool onlyLogKFPoses, bool saveMetricPoses, bool useCamToTrackingRef)
    {
        boost::unique_lock<boost::mutex> lock(trackMutex);
        boost::unique_lock<boost::mutex> crlock(shellPoseMutex);
    ...
    std::pair<Vec4, bool> FullSystem::trackNewCoarse(FrameHessian* fh, Sophus::SE3d* referenceToFrameHint)
    {
        dmvio::TimeMeasurement timeMeasurement(
            referenceToFrameHint ? "FullSystem::trackNewCoarse" : "FullSystem::trackNewCoarseNoIMU");
        ...
        if (referenceToFrameHint)
        {
            ...
            lastF_2_fh_tries.push_back(*referenceToFrameHint);
            {
                ...
                for (int i = allFrameHistory.size() - 2; i >= 0; i--)
                {
                    ...
                    {
                        std::cout << "WARNING: No well tracked frame with the same tracking ref available!"
                            << std::endl;
        ...
        if (!haveOneGood)
        {
            ...
            bool matrix_contains_nan = false;
            // Get pointer to the matrix data (adjust if needed: .data(), .ptr(), .m, or implicit conversion)
            const GLdouble* matrix_data = lastF_2_fh.matrix().data();
            for (int i = 0; i < 16; ++i)
            {
                // A 4x4 matrix has 16 elements
                if (std::isnan(matrix_data[i]))
                {
                    matrix_contains_nan = true;
                    break; // Found one, no need to check the rest
                }
            }
            if (lastF_2_fh.translation().norm() > 100000 || matrix_contains_nan)
            {
    ...
    void FullSystem::addActiveFrame(ImageAndExposure* image, int id, dmvio::IMUData* imuData, dmvio::GTData* gtData)
    {
        ...
        else // do front-end operation.
        {
            ...
            if (needToMakeKF)
            {
                ...
                if (framesBetweenKFs <
                    (int)setting_minFramesBetweenKeyframes) // if integer value is smaller we just skip.
                {
    ...
    void FullSystem::mappingLoop()
    {
        ...
        while (runMapping)
        {
            ...
            if (!setting_debugout_runquiet)
            {
                std::cout << "Current mapping id: " << fh->shell->id << " create KF after: " << needNewKFAfter
                    << std::endl;
            }
